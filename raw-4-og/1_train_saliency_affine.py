...
class DualViewMammogramDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        side = str(row['breast_side']).strip().upper() # Get the breast side

        # Load as Grayscale PIL Images
        cc_image = Image.open(cc_path).convert('L')
        mlo_image = Image.open(mlo_path).convert('L')

        # CRITICAL STN PREPROCESSING: Align orientations
        # All breasts must face the same direction (e.g., facing Right).
        # Typically, Left breasts face right naturally. Right breasts face left.
        if side == 'RIGHT':
            cc_image = TF.hflip(cc_image)
            mlo_image = TF.hflip(mlo_image)

        # Apply standard transforms (Resize, Tensor, Normalize)
        cc_tensor = self.transform(cc_image)
        mlo_tensor = self.transform(mlo_image)

        return cc_tensor, mlo_tensor, torch.tensor(label, dtype=torch.long), cc_path, mlo_path
...
class RawSpatialTransformer(nn.Module):
    ...
    def forward(self, cc_view, mlo_view):
        ...
        # Stop the STN from squishing the image into a tiny dot
